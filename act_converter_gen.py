...
if __name__ == "__main__":
        ...
        for epoch in tqdm(range(args.epoch)):
            ...
            with torch.no_grad():
                ...
                for val_item in tqdm(valid_dataset):
                    ...
                    print(val_target)
                    print("_____________")
                    val_agg_score+=bert_score_metric(pred_str, val_target, lang=target_lang_code)
                    h_status = []
                val_acc = val_agg_score/len(valid_dataset)
    ...
    with torch.no_grad():
        for test_item in tqdm(test_dataset):
            ...
            print(test_target)
            print("____________")
            test_agg_score+=bert_score_metric(pred_str, test_target, lang=target_lang_code)
            # act_convert_res.append({"prompt": test_prompt, "gt": test_target, "pred": pred_str, "prob":pred_probs})
            act_convert_res.append({"prompt": test_prompt, "gt": test_target, "pred": pred_str})
            h_status = []
        interv_acc = test_agg_score/len(test_dataset)
        print(interv_acc)

...
if __name__ == "__main__":
    ...
    parser.add_argument("--result_folder", type=str, default='./KD_result')
    ...
    for l in label_list:
        train_buckets[l] = random.sample(train_buckets[l], 3)
    ...
    if os.path.exists(os.path.join(res_dir, "head_act.pt")):
        ...
        label_prob_dict = torch.load(os.path.join(res_dir, "label_prob_dict.pt"))
    else:
        activation_storage = []
        label_prob_dict = {l:[] for l in label_list}
        softmax = nn.Softmax(dim=-1)
        for l in tqdm(label_list):
            for dummy_query in train_buckets[l]:
                ...
                while cnt<10:
                    ...
                    dummy_output_sequence = dummy_output.sequences
                    dummy_output_logits = dummy_output.logits
                    dummy_pred_str = tokenizer.decode(dummy_output_sequence.squeeze()[len(dummy_tokenized_input.input_ids.squeeze()):], skip_speical_tokens=True)
                    ...
                    print(dummy_pred_str)
                    print(dummy_query_target)
                    print("____________")
                    if dummy_pred_str==dummy_query_target:
                        ...
                        label_prob_dict[l].append([softmax(logit).squeeze().detach().cpu() for logit in dummy_output_logits])
                        break
                    else:
                        cnt+=1
            label_prob_dict[l] = torch.mean(torch.stack([torch.stack(item) for item in label_prob_dict[l]]), dim=0)
        for k,v in label_prob_dict.items():
            if len(v)==0:
                print(k)
        ...
        torch.save(label_prob_dict, os.path.join(res_dir, "label_prob_dict.pt"))
            ...
            with torch.set_grad_enabled(True):
                ...
                for idx, batch_start in tqdm(enumerate(range(0, len(train_dataset), args.batch_size))):
                    ...
                    for train_item in train_items:
                        ...
                        batched_target_icl_probs.append(label_prob_dict[train_target])
                    batched_target_icl_probs = torch.cat(batched_target_icl_probs, dim=0)
                    ...
                    ce_loss_fn = torch.nn.CrossEntropyLoss(reduction="none", ignore_index = -100)
                    ce_loss = ce_loss_fn(out_logit.reshape(-1,out_logit.shape[-1]), batched_train_target.reshape(-1).to(device)).mean()
                    kl_loss_fn = nn.KLDivLoss(reduction="none")
                    valid = (batched_train_target!=-100)
                    selected = F.log_softmax(out_logit[valid], dim=-1)
                    kl_loss = kl_loss_fn(selected, batched_target_icl_probs.to(device)).sum(dim=-1)
